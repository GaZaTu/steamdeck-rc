...
private:
  ...
  // void SpiWrite(uint32_t u32_SPI_Data);

  // void Rtc6705_Write(uint8_t u8_Addr, uint32_t u32_Value);
...
public:
  void begin(int8_t sck = -1, int8_t data = -1, int8_t ss = -1, int8_t rssi = -1) {
    ...
    pinMode(data, OUTPUT);
    pinMode(ss, OUTPUT);
    pinMode(rssi, INPUT_PULLUP);

    // _sck = sck;
    // _data = data;
    // _ss = ss;
    _spi.begin(sck, -1, data, ss);
  ...
  // void Rtc6705_SetFreq(uint8_t Group, uint8_t ch);
  ...
  void setFrequency(uint32_t freq) {
    ...
    uint8_t data0 = DelitelL * 32 + 17;
    uint8_t data1 = DelitelH * 16 + DelitelL / 8;
    uint8_t data2 = DelitelH / 16;
    uint8_t data3 = 0;
